...
class BlockBasedMetaBuilder::BlockBasedTablePropertiesCollector
...
struct BlockBasedMetaBuilder::Rep {
...
BlockBasedMetaBuilder::BlockBasedMetaBuilder(
...
void BlockBasedMetaBuilder::Add(const Slice& key, const Slice& value) {
  Rep* r = rep_;
  assert(rep_->state != Rep::State::kClosed);
  if (!ok()) return;
...
void BlockBasedMetaBuilder::Flush() {
  Rep* r = rep_;
  assert(rep_->state != Rep::State::kClosed);
  if (!ok()) return;
...
Status BlockBasedMetaBuilder::status() const { return rep_->status; }
...
//
Status BlockBasedMetaBuilder::InsertBlockInCache(const Slice& block_contents,
...
void BlockBasedMetaBuilder::WritePropertiesBlock(
  ...
  if (ok()) {
#ifndef NDEBUG
    {
      ...
      TEST_SYNC_POINT_CALLBACK(
          "BlockBasedMetaBuilder::WritePropertiesBlock:GetPropsBlockOffset",
          &props_block_offset);
      TEST_SYNC_POINT_CALLBACK(
          "BlockBasedMetaBuilder::WritePropertiesBlock:GetPropsBlockSize",
          &props_block_size);
    }
#endif  // !NDEBUG
...
void BlockBasedMetaBuilder::WriteCompressionDictBlock(
  ...
  if (rep_->compression_dict != nullptr &&
    ...
    if (ok()) {
      ...
      TEST_SYNC_POINT_CALLBACK(
          "BlockBasedMetaBuilder::WriteCompressionDictBlock:RawDict",
          &compression_dict);
#endif  // NDEBUG
    }
    if (ok()) {
...
void BlockBasedMetaBuilder::WriteFooter(BlockHandle& metaindex_block_handle,
  ...
  // this is guaranteed by BlockBasedMetaBuilder's constructor
...
void BlockBasedMetaBuilder::EnterUnbuffered() {
  Rep* r = rep_;
...
Status BlockBasedMetaBuilder::Finish() {
  Rep* r = rep_;
...
void BlockBasedMetaBuilder::Abandon() {
  assert(rep_->state != Rep::State::kClosed);
  rep_->state = Rep::State::kClosed;
}

uint64_t BlockBasedMetaBuilder::NumEntries() const {
...
TableProperties BlockBasedMetaBuilder::GetTableProperties() const {
